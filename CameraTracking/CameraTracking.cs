...
public class CameraTracking : WebCamera
{
    ...
    //processor.Faces
        ...
        detected = processor.Faces.Count > 0;
        if (detected)
        {
            DetectedFace face = processor.Faces[0];        
            int center = input.width / 2;
            int diff = face.Region.Right - face.Region.Left;
            int offset = face.Region.Left + diff / 2;
